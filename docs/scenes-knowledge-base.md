...
| 2026-08-04 | **Superseding the 2026-07-28 entry:** Ticketmaster is now a **long-term partner source**, not a throwaway kickstart — we're positioning Scenes as a Ticketmaster Discovery API partner rather than dropping it before launch. See `ingestion_ticketmaster.md` §2. | We already satisfy the Discovery API's compliance gate for real: every event carries `ticketSource`/`ticketUrl`, and `SourceAttribution.tsx` renders a visible "Billets via Ticketmaster ↗" badge that IS the outbound buy-link, on every show page where TM supplied data — not an afterthought. That closes the original objection (proprietary data, ToS requires attribution + traffic flowing back), so a permanent ingestion relationship is viable; worth pursuing the rate-limit-increase / partner conversation with Ticketmaster. |
...
| 2026-08-11 | **Event-detail bootstrap run and retired**: the one-time script above was run against staging across 7 batches over several days, enriching roughly **160 distinct upcoming events** (poster/author/director/duration/cast; sourced mostly from billetreduc's JSON-LD, some from venue sites, none from theatre.info or ticketmaster.fr). Field coverage across the ~624 events with a performance in the next 60 days went from near-zero to 122 with a poster, 77 with an author, 39 with a director, 58 with a duration, 158 with at least one linked cast member. Remaining gaps are mostly small café-théâtre/one-person shows with no public page on any of the three sources, and events too far in the future to have a published show page yet — both correctly skipped rather than guessed. One research-agent mismatch (wrong billetreduc show attached to an eventId, caught via a per-batch name/source cross-check before applying) confirms the value of that manual verification step for any future rounds. Per the plan, the script is now considered done; it is not scheduled and not wired into `apps/worker`. | Confirms the bounded-read legal framing held up in practice — every write passed the re-fetch/substring verification safety net, and research agents self-reported skips (anti-bot blocks, mismatched-city same-name shows, no accessible page) rather than fabricating data. |
